"""Task offers: a task broadcast to the crowd by email, first come first served.

The SQL is copied verbatim from db/130_task_offers.sql so the bootstrap and
migration paths keep producing identical schemas (make verify-schema). RLS
policies are hand-written, as in 0003 - autogenerate does not see them.

asyncpg prepares each statement, so the file is executed one statement at a
time: split on a semicolon at end of line, except inside the $fn$ body.

Deferred: an RLS isolation test for task_offer / task_offer_recipient belongs
in tests/isolation/ once the conftest fixtures exist.
...
def downgrade() -> None:
    op.execute("DROP TABLE task_offer_recipient")
    op.execute("DROP TABLE task_offer")
    op.execute("DROP FUNCTION task_offer_lookup(text)")
    op.execute("DROP TYPE task_offer_response")
    op.execute("DROP TYPE task_offer_status")
