"""An organisation can add its own people.

The capabilities existed since 900_seed — user.invite, user.manage,
role.manage, granted to six roles — and nothing had ever read them. This adds
the floor under an endpoint that finally does.

Most of it is a security floor rather than a feature. applies_to_kind was
enforced by nothing, and user_role_grant_write constrains org_id but not
role_id, so the obvious invite endpoint would have let any client-org member
grant themselves platform_admin — and app.role is what is_platform_admin()
reads. That is closed here by a trigger, not at the endpoint, because an
endpoint check is one forgotten call site from being undone.

user_role_grant_live_key is narrowed to (user_id, org_id): with role_id in the
key a role change left two live grants, user_capabilities unioned both
permission sets, and which role reached the JWT was a tie-break.

invitation_accept is replaced so acceptance cannot overwrite an existing
password. It was safe only while every invitation pointed at a brand-new user,
which is exactly what this migration changes.

No new table, no new column, no new policy.

SQL copied verbatim from db/200_org_members.sql so the bootstrap and migration
paths keep producing identical schemas (make verify-schema); a test asserts the
two stay byte-identical.

Executed one statement at a time, with $fn$ bodies held together — the same
splitter 0012 to 0014, 0018, 0019 and 0020 use. 0017 sat unapplied on the pilot
...
DROP FUNCTION IF EXISTS assert_org_has_owner();

DROP FUNCTION IF EXISTS user_role_grant_role_fits_org();

DROP FUNCTION IF EXISTS invite_member(citext,text,text,grant_scope,text,interval);

DROP FUNCTION IF EXISTS revoke_member_sessions(uuid);

DROP FUNCTION IF EXISTS org_member_by_email(citext);
