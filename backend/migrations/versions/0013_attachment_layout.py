"""Attachment layout: a grouped tree, self-describing names, numbered versions.

The SQL is copied verbatim from db/140_attachment_layout.sql so the bootstrap
and migration paths keep producing identical schemas (make verify-schema).

asyncpg prepares each statement, so the file is executed one statement at a
time: split on a semicolon at end of line, except inside the $fn$ body — the
same splitter 0012 uses.

attachment_folder() changes its OUT columns, which CREATE OR REPLACE cannot do,
so it is dropped and created again with its REVOKE and GRANT restated. The
downgrade puts the two-column version from 0007 back, grants included.
...
def upgrade() -> None:
    for stmt in _statements(_DDL):
        op.execute(stmt)


def downgrade() -> None:
    # Storage keys written under the new layout stay where they are: every read
    # goes through attachment.storage_key, so the old code opens them fine.
    op.execute("DROP FUNCTION attachment_folder(attachment_entity, uuid)")
    ...
    op.execute("DROP INDEX attachment_doc_version_key")
    op.execute("ALTER TABLE attachment DROP COLUMN version, DROP COLUMN doc_no")
