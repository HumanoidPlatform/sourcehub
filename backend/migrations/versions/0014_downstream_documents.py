"""The client's working documents reach aggregators and workers.

The SQL is copied verbatim from db/150_downstream_documents.sql so the
bootstrap and migration paths keep producing identical schemas
(make verify-schema).
...
same splitter 0012 and 0013 use.

Purely additive: two SECURITY DEFINER helpers and one extra PERMISSIVE select
policy on attachment. attachment_select is not touched, and no row changes.
...
def downgrade() -> None:
    op.execute("DROP POLICY attachment_select_downstream ON attachment")
    op.execute("DROP FUNCTION task_request_id(uuid)")
    op.execute("DROP FUNCTION request_shared_downstream(uuid)")
