...
describe("applyResult", () => {
  it("records the presign and resets the attempt counter", () => {
    const p = applyResult({ ...base, attempts: 3 }, { type: "presigned", asset_id: "a", url: "u", expires_in: 900, status: "pending" }, NOW);
  ...
  it("skips the PUT when the server already holds the file", () => {
    const p = applyResult(base, { type: "presigned", asset_id: "a", url: null, expires_in: 0, status: "ready" }, NOW);
